		...
		<div class="row justify-content-center">
			<div class="col-12">
				<form class="form-inline" id="unidadeForm">
					<div class="col p-0">
						<label class="sr-only" for="contatoAssunto">Assunto:</label> <input
							type="text" name="busca" value="${busca}"
							class="form-control mb-2 mr-sm-2 form-control-sm" id="assunto"
							placeholder="Ex.: olimpíadas"> <input
							<c:if test="${estaRespondido == true}"> checked="checked" </c:if>
							name="estaRespondido" type="checkbox"
							class="form-control mb-2 mr-sm-2 form-control-sm form-check-input"
							id="estaRespondido"> <span> Respondidos</span>
			...
			<div class="col-12">
				<div class="table-responsive">
					<table class="table table-nowrap" id="contatoTable">
						<thead>
							<tr>
								<th>Nome</th>
								<th>E-mail</th>
						...
						<tbody>
							<c:forEach items="${paginacao.objetosDaPaginaAtual}"
								...
								<tr>
									...
									<td><c:if test="${contato.estaRespondido == true}">
											<label class="badge badge-pill badge-success p-2 pl-3 pr-3">Sim</label>
										</c:if> <c:if test="${contato.estaRespondido == false}">
											<label class="badge badge-pill badge-danger p-2 pl-3 pr-3">Não</label>
										</c:if></td>
									<td>${contato.respondidoPor.nome}</td>
									<td>${contato.dataDoEnvio}</td>
									<td>${contato.dataDeResposta}</td>
									<td><a href="javascript:void(0)"
										data-message="Você tem certeza que deseja apagar?"
										data-url="${sessao.urlPadrao}adm/contatos/${contato.id}/apagar"
										class="btn btn-small btn-outline-warning button-remove"><i
											class="fa-fw far fa-trash"></i></a></td>
									<td><a
										href="${sessao.urlPadrao}adm/contatos/${contato.id}/editar"
		...
		<nav aria-label="Navegação de página exemplo"
			...
			<ul class="pagination justify-content-end">
				<li
					...
					<a class="page-link"
					href="${sessao.urlPadrao}adm/contatos?paginaAtual=${paginacao.paginaAtual - 1}&busca=${busca}"
					tabindex="-1">Anterior</a>
				</li>
				<li class="page-item"><select class="page-link button-paginate">
						<c:forEach items="${paginacao.itens}" var="item">
							<option
								<c:if test="${item.ehPaginaAtual() == true}">selected</c:if>
								value="${item.numeroDaPagina}"
								data-link="${sessao.urlPadrao}adm/contatos?paginaAtual=${item.numeroDaPagina}&busca=${busca}">${item.numeroDaPagina}
				...
				<li
					...
					<a class="page-link"
					href="${sessao.urlPadrao}adm/contatos?paginaAtual=${paginacao.paginaAtual + 1}&busca=${busca}">Próximo</a>
				</li>
			</ul>
		</nav>
	</div>
